...
// search functions
class Search {
	// describe and create/initiate the object
	constructor(){
		this.addSearchHTML();
	...
	typingLogic(){
		...
		this.searchTimer = setTimeout(this.getResults.bind(this), 700);
			} else {
	...
	getResults(){
		$.getJSON(MSPData.root_url + '/wp-json/msp/v1/search?term=' + this.searchField.val(), (results) => {
			this.resultsDiv.html(`
				<div class="row">
					<div class="one-third">
						<h2 class-"search-overlay__section-title">Pages</h2>
						${results.pageInfo.length ? '<ul class="link-list min-list">' : '<p>No results found</p>'}
					...
					<div class="one-third">
						...
						${results.jobs.length ? '<ul class="link-list min-list">' : `<p>No results found<br/> <a href="${MSPData.root_url}/jobs">View all jobs</a></p>`}
						${results.jobs.map(item => `<li><a href="${item.permalink}">${item.title}</a><br/>- posted on ${item.postedOn}</li>`).join('')}
						${results.jobs.length ? '</ul>' : ''}
					</div>
					<div class="one-third">
						<h2 class-"search-overlay__section-title">News</h2>
						${results.news.length ? '<ul class="link-list min-list">' : `<p>No results found<br/> <a href="${MSPData.root_url}/news">View all News</a></p>`}
	...
	openOverlay(){
		...
		setTimeout(() => this.searchField.focus(), 801);
	}
